...
export class OrderService {
  ...
  async saveUserCachedLastBillingAddress(
    uid: string,
    shipping: OrderShippingModel
  ): Promise<OrderShippingModel> {
    if (typeof uid !== "string") {
    ...
    if (
      ...
      await cache({ database: "e-commerce-cache", collection: "shipping" }).set(
        uid,
        shipping
      );
      return shipping;
